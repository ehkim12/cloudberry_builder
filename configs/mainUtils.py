...
def check_fts(fts):
    ...
    try:
        ...
        # gpssh might fail, assume no FTS process is running
        return False
    except (ValueError, subprocess.TimeoutExpired):
        # Error parsing output or timeout
        return False
...
def start_etcd(etcd, etcd_home_num, etcd_host_list):
    ...
    try:
        ...
        print(f"[WARNING] Error starting ETCD on {etcd}: {e}")
        raise
...
def start_fts(fts, isdemo):
    ...
    try:
        ...
        print(f"[WARNING] Error starting FTS on {fts}: {e}")
        raise
...
def kill_fts(fts):
    ...
    try:
        ...
        # pkill returns 1 when no processes are found - this is normal
        if e.returncode == 1:
            print(f"[INFO] No FTS processes found on {fts}")
        else:
            print(f"[WARNING] Error killing FTS process on {fts}: {e}")
...
def kill_etcd(etcd):
    ...
    try:
        ...
        # pkill returns 1 when no processes are found - this is normal
        if e.returncode == 1:
            print(f"[INFO] No ETCD processes found on {etcd}")
        else:
            print(f"[WARNING] Error killing ETCD process on {etcd}: {e}")
    except subprocess.TimeoutExpired:
...
def resolve_hostname(hostname):
    ...
    try:
        ...
        # fallback to returning the hostname as-is
        return hostname 
...
def start_single_etcd(etcd):
    ...
    try:
        ...
        print(f"[WARNING] Error starting single ETCD on {etcd}: {e}")
        raise
