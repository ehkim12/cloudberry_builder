...
OUTPUT_HOST_TO_FILE() {
    ...
    # Use temporary config directory during initialization to avoid conflicts with initdb
    local config_dir="/tmp/cbdb_init_config"
    
    # Check if we're in initialization mode - if the coordinator directory doesn't have database files yet
    if [ -n "${COORDINATOR_DIRECTORY}" ] && [ -n "${SEG_PREFIX}" ]; then
        local coord_dir="${COORDINATOR_DIRECTORY}/${SEG_PREFIX}-1"
        if [ -d "$coord_dir" ] && [ -f "$coord_dir/PG_VERSION" ]; then
            # Database is initialized, use normal config location
            config_dir="${coord_dir}/config"
        fi
    fi
...
SETUP_ETCD () {
    ...
    if [ "$ETCD_START_MODE" = "DEMO" ];then
        ...
        #ETCD_START_CMD="nohup etcd > /tmp/etcd.log 2>&1 &"
		gpssh -h $ETCD_HOST -e "${ETCD_START_CMD}"
		LOG_MSG "[INFO]: ETCD CMD : $ETCD_START_CMD on host $ETCD_HOST"
    fi
...
SETUP_FTS() {
    ...
    # Use temporary FTS log directory during initialization to avoid conflicts with initdb
    ACTUAL_FTS_LOG_DIR="${FTS_LOG_DIR}"
    if [ "$FTS_START_MODE" != "DEMO" ]; then
        # During initialization, use a temporary location for FTS logs
        ACTUAL_FTS_LOG_DIR="/tmp/fts_init_logs/${FTS_HOST}"
    fi
    
    if [ "$FTS_START_MODE" = "DEMO" ];then
        nohup $GPHOME/bin/gpsync -F $GPHOME/bin/config/cbdb_etcd_default.conf -d ${ACTUAL_FTS_LOG_DIR}/log/fts -D -C -a >/dev/null 2>&1 &
    else
        # Use the etcd configuration file with proper endpoints
        if [ -f "/tmp/cbdb_etcd.conf" ]; then
            ETCD_CONFIG_TO_USE="/tmp/cbdb_etcd.conf"
        else
            ETCD_CONFIG_TO_USE=$ETCD_CONFIG_FILE_PATH
        fi
        gpsync -h ${FTS_HOST} $ETCD_CONFIG_TO_USE =:${ETCD_CONFIG_TMP_FILE}
        FTS_CMD="mkdir -p ${ACTUAL_FTS_LOG_DIR}/log/fts && nohup ${GPHOME}/bin/gpfts -F ${ETCD_CONFIG_TMP_FILE} -d ${ACTUAL_FTS_LOG_DIR}/log/fts >/dev/null 2>&1 &"
        ssh -f -o "BatchMode yes" -o "StrictHostKeyChecking no" ${FTS_HOST} "${FTS_CMD}"
    fi
    OUTPUT_HOST_TO_FILE $FTS_HOST_FILE $FTS_HOST
    LOG_MSG "[INFO]: FTS started on host $FTS_HOST"
}
...
CHECK_FTS () {
    ...
    # Check if FTS process is running
    FTS_CHECK_CMD="ps -ef | grep -i 'gpfts' | grep -v grep"
    FTS_PROCESS_RES=`gpssh -h ${FTS_HOST} -e "${FTS_CHECK_CMD}" 2>/dev/null | grep "/bin/gpfts" | wc -l`
    
    if [ "$FTS_PROCESS_RES" -ge "1" ]; then
        # FTS process is running, check if it has active logs (indicating it's working)
        FTS_LOG_CHECK=`gpssh -h ${FTS_HOST} -e "find /data*/database/*/log/fts -name '*.csv' -mtime -1 2>/dev/null | head -1" 2>/dev/null | grep -v "^\[" | head -1`
        if [ -n "$FTS_LOG_CHECK" ]; then
            ret=0  # FTS is running and has active logs
            LOG_MSG "[INFO]: FTS process found and active on $FTS_HOST"
        else
            # Check if FTS process is very new (less than 30 seconds old)
            FTS_NEW_PROCESS=`gpssh -h ${FTS_HOST} -e "ps -eo pid,etime,cmd | grep gpfts | grep -v grep | awk '\$2 ~ /^[0-9][0-9]?$/ {print \$1}'" 2>/dev/null | grep -v "^\[" | wc -l`
            if [ "$FTS_NEW_PROCESS" -ge "1" ]; then
                ret=0  # FTS process is very new, give it time
                LOG_MSG "[INFO]: FTS process found on $FTS_HOST (recently started)"
            else
                ret=0  # FTS process exists, assume it's working
                LOG_MSG "[INFO]: FTS process found on $FTS_HOST"
            fi
        fi
    else
        ret=1  # FTS is not running - return 1 to indicate "success/can proceed"
        LOG_MSG "[INFO]: FTS process not found on $FTS_HOST, will start FTS"
    fi
    return $ret
}
